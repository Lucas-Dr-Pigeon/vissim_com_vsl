...
class Links:
    ...
    def _output(self, links):
        ...
        def _out(label, s=True):
            a = self.get(linkNum, label, string=s)
            return str(a)
        ...
        print (vissimOut)
        print(_out('from', s=False))
        print (_out('over', s=False))
        print(_out('to', s=False))

        if _out('over', s=False):
            overStr = ''

            for no, i in enumerate(_out('over', s=False)):
                print (str(no) + ": " + str(i))
